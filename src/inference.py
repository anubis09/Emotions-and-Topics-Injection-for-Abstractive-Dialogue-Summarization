...
parser.add_argument(
    ...
    type=str,
    default="./new_weights_comet/final_Trial1_context_comet.txt",
)
parser.add_argument(
...
if args.train_configuration == "base":
    finetune_model = BartForConditionalGeneration.from_pretrained(
    ...
    finetune_model = BartForConditionalGeneration_DualDecoder.from_pretrained(
        args.model_checkpoint
    )
...
bertscore_metric = load_metric(
    "bertscore", lang="en", model_type="bert-base-uncased"
)
if args.dataset_name == "dialogsum":
    bertscore_metric2 = load_metric(
        "bertscore", lang="en", model_type="bert-base-uncased"
    )
    bertscore_metric3 = load_metric(
        "bertscore", lang="en", model_type="bert-base-uncased"
    )
...
with torch.no_grad():
    for idx, data in enumerate(tqdm(test_dataloader), 0):
        ...
        if args.dataset_name == "dialogsum":
            ...
            decoded_labels2 = [
                "\n".join(nltk.sent_tokenize(label.strip()))
                for label in decoded_labels2
            ]
            decoded_labels3 = [
                "\n".join(nltk.sent_tokenize(label.strip()))
                for label in decoded_labels3
            ]

            result2 = metric2.add_batch(
                predictions=decoded_preds, references=decoded_labels2
            )
            result3 = metric3.add_batch(
                predictions=decoded_preds, references=decoded_labels3
            )

            bertscore_metric2.add_batch(
                predictions=decoded_preds, references=decoded_labels2
            )
            bertscore_metric3.add_batch(
                predictions=decoded_preds, references=decoded_labels3
            )
...
if args.dataset_name == "dialogsum":
    ...
    bertscore_result2 = bertscore_metric2.compute(
        lang="en", model_type="bert-base-uncased"
    )
    bertscore_result3 = bertscore_metric3.compute(
        lang="en", model_type="bert-base-uncased"
    )
...
if args.dataset_name == "dialogsum":
    bertscore_result2 = sum(bertscore_result2["f1"]) / len(
        bertscore_result2["f1"]
    )
    bertscore_result3 = sum(bertscore_result3["f1"]) / len(
        bertscore_result3["f1"]
    )
